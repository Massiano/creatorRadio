...
def diag():
    ...
    try:
        ...
        sched = compute_schedule(tracks, total_duration, now, skip_offset)
        playlist_status = {"status": "ok", "file_path": PLAYLIST_PATH, "exists": True, "track_count": len(tracks), "total_duration_sec": total_duration, "schedule_version": version, "now_playing": sched["current"]["title"], "all_tracks": [{"id": t["id"], "title": t["title"], "duration": t["duration"]} for t in tracks]}
    except Exception as e:
        playlist_status = {"status": "error", "file_path": PLAYLIST_PATH, "exists": os.path.exists(PLAYLIST_PATH), "error": str(e), "track_count": 0, "all_tracks": []}
    return jsonify({"service": "Creator Radio Backend", "status": playlist_status["status"], "playlist": playlist_status, "system": {"python": sys.version.split()[0], "platform": platform.platform(), "port": int(os.environ.get("PORT", 5000))}}), (200 if playlist_status["status"] == "ok" else 503)
